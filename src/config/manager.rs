...
use crate::config::storage::{Storage, DefaultConfigStorage};
...
pub trait Manager<D = ConfigData, E = ConfigError> {
...
mod tests {
    ...
    fn test_get_data() {
        ...
        let data = config_manager.get_data();
    ...
    mod config_storage_new {
        ...
        fn test_returns_error() {
            ...
            mock_storage.expect_load().returning(|| {
                Err(ConfigError::Io(IoError::new(IoErrorKind::Other, "テストエラー")))
            });
        ...
        fn test_load_once_called() {
            ...
            mock_storage.expect_load().once().returning(|| {
                Ok(ConfigData::default())
            });
