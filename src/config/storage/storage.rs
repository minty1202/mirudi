use crate::config::storage::serializer::{
    Serializer,
    YamlSerializer
};
...
use crate::config::storage::filesystem::{
    FileSystem,
    OsFileSystem
};
...
mod tests {
    ...
    fn test_get_file_path() {
        ...
        mock_fs.expect_create_dir_all()
            .returning(|_| Ok(())); 
        mock_fs.expect_create_file()
            .returning(|_| Ok(()));
        let storage = ConfigStorage::new(config_path.clone(), Arc::new(mock_fs), Arc::new(MockSerializer::new())).unwrap();
    ...
    mod config_storage_new {
        ...
        fn test_config_storage_new_returns_ok() {
            ...
            mock_fs.expect_create_file()
                .returning(|_| Ok(()));
            let result = ConfigStorage::new(config_path, Arc::new(mock_fs), Arc::new(MockSerializer::new()));
        ...
        fn test_config_storage_new_creates_directory() {
            ...
            let _ = ConfigStorage::new(config_path, Arc::new(mock_fs,), Arc::new(MockSerializer::new()));
        }
    ...
    mod config_storage_save {
        ...
        fn test_config_storage_save() {
            ...
            mock_serializer.expect_serialize()
            ...
            let storage = ConfigStorage::new(config_path, Arc::new(mock_fs), Arc::new(mock_serializer)).unwrap();
        ...
        fn test_config_storage_save_invalid_data() {
            ...
            mock_serializer.expect_serialize::<ConfigData>()
        ...
        fn test_config_storage_save_file_system_error() {
            ...
            mock_fs.expect_write_file()
                ...
                .returning(|_, _| Err(std::io::Error::new(std::io::ErrorKind::Other, "File system error")));
    ...
    mod config_storage_load {
        ...
        fn test_config_storage_load() {
            ...
            mock_serializer.expect_deserialize()
        ...
        fn test_config_storage_load_file_system_error() {
            ...
            mock_fs.expect_read_to_string()
                .with(mockall::predicate::eq(config_path.clone()))
                .returning(|_| Err(std::io::Error::new(std::io::ErrorKind::Other, "File system error")));

            mock_serializer.expect_deserialize::<ConfigData>()
                .times(0);
